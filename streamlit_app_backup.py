...
if st.session_state["page"] == "Home":
    ...
    for device_id in device_data:
        ...
        with col1:
            st.write(device_info["device"]["enabled"])
        with col2:
            st.write(device_info["device"]["device_id"])
        with col3:
            st.write(device_info["device"]["device_name"])
        with col4:
            st.write(device_info["device"]["host"])
        with col5:
            st.write(device_info["device"]["protocol_name"])

        # Mostrar el estado de la conexión
        with col6:
            status_color = (
                "green" if  device_info["device"]["connection_status"] == "Good" else
                "red" if device_info["device"]["connection_status"] == "Failure" else "gray"
            )
            st.markdown(
                f"<span style='color: {status_color};'>{device_info['device']['connection_status']}</span>",
